...
export function CompassPage() {
    ...
    const saveSnapshot = async () => {
        try {
            ...
            if (result.ok) {
                setAlertDialog({ isOpen: true, title: 'Success', message: 'Snapshot saved successfully!', type: 'success' });
                // Refresh the recent snapshots list
                await loadRecentSnapshots();
            } else {
                setAlertDialog({ isOpen: true, title: 'Error', message: `Failed to save snapshot: ${result.error}`, type: 'error' });
            }
        } catch (err) {
            console.error('Failed to save snapshot:', err);
            setAlertDialog({ isOpen: true, title: 'Error', message: 'Failed to save snapshot.', type: 'error' });
        }
    };
