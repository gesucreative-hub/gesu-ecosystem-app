...
import { Briefcase, Target, Play, Square, Trash2, Database, ChevronDown, ChevronUp, ChevronRight, Zap, Info, X, Rocket, Plus, ClipboardList, ArrowRight, RefreshCw, History, TrendingUp, Activity } from 'lucide-react';
...
export function CompassPage() {
    ...
    // Tab-Based Layout State (Option 3: Tab-Based Workspace)
    ...
    return (
        <>
            <PageContainer>
                ...
                {/* TAB NAVIGATION (Option 3: Tab-Based Workspace) */}
                <div className="mb-6 border-b border-tokens-border">
                    <div className="flex gap-1 -mb-px">
                        {[
                            { id: 'today' as const, label: t('tabs.today', '📋 Today'), desc: 'Tasks & Planning' },
                            { id: 'energy' as const, label: t('tabs.energy', '⚡ Energy'), desc: 'Calibration' },
                            { id: 'focus' as const, label: t('tabs.focus', '🎯 Focus'), desc: 'Start Session' },
                            { id: 'history' as const, label: t('tabs.history', '📊 History'), desc: 'Snapshots' }
                        ].map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`
                                    relative px-5 py-3 text-sm font-medium transition-all rounded-t-lg
                                    ${activeTab === tab.id 
                                        ? 'text-tokens-brand-DEFAULT bg-tokens-panel border border-tokens-border border-b-tokens-panel -mb-px z-10' 
                                        : 'text-tokens-muted hover:text-tokens-fg hover:bg-tokens-panel2/50'}
                                `}
                            >
                                <span className="flex flex-col items-center gap-0.5">
                                    <span>{tab.label}</span>
                                    {activeTab === tab.id && (
                                        <span className="text-[10px] text-tokens-muted">{tab.desc}</span>
                                    )}
                                </span>
                                {/* Active indicator */}
                                {activeTab === tab.id && (
                                    <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-tokens-brand-DEFAULT" />
                                )}
                            </button>
                        ))}
                    </div>
                                ...
                                {!timerActive && (
                                    <>
                                        <div className="space-y-3">
                                            <label className="text-sm font-bold text-tokens-muted uppercase tracking-wider">
                                                {t('focusFirst.step1')}
                                            </label>
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={() => setFocusFirstMode('select')}
                                                    className={`flex-1 py-3 px-4 rounded-lg border text-sm font-medium transition-all ${
                                                        focusFirstMode === 'select' 
                                                            ? 'border-tokens-brand-DEFAULT bg-tokens-brand-DEFAULT/10 text-tokens-brand-DEFAULT' 
                                                            : 'border-tokens-border text-tokens-muted hover:border-tokens-brand-DEFAULT/50'
                                                    }`}
                                                >
                                                    {t('focusFirst.selectFromList')}
                                                </button>
                                                <button
                                                    onClick={() => setFocusFirstMode('text')}
                                                    className={`flex-1 py-3 px-4 rounded-lg border text-sm font-medium transition-all ${
                                                        focusFirstMode === 'text' 
                                                            ? 'border-tokens-brand-DEFAULT bg-tokens-brand-DEFAULT/10 text-tokens-brand-DEFAULT' 
                                                            : 'border-tokens-border text-tokens-muted hover:border-tokens-brand-DEFAULT/50'
                                        ...
                                        <div className="space-y-3">
                                            <label className="text-sm font-bold text-tokens-muted uppercase tracking-wider">
                                                {t('focusFirst.step2')} <span className="font-normal">(optional)</span>
                                            </label>
                                            <input
                                                type="text"
                                                value={microStep}
                                                onChange={(e) => setMicroStep(e.target.value)}
                                                placeholder={t('focusFirst.step2Placeholder')}
                                                className="w-full px-4 py-3 text-sm bg-tokens-bg border border-tokens-border rounded-lg focus:outline-none focus:ring-2 focus:ring-tokens-ring text-tokens-fg placeholder:text-tokens-muted/50"
                                            />
                                        </div>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        <Card title={
                            <div className="flex items-center justify-between w-full">
                                ...
                                <Badge variant="neutral">{recentSnapshots.length} {t('snapshot.sessions')}</Badge>
                            </div>
                        }>
                            {snapshotsLoading ? (
                        ...
                        <Card title={
                            <div className="flex items-center gap-2">
                                ...
                                <span>{t('insights.energyTrend')}</span>
                            </div>
                        }>
                            <div className="flex justify-center py-4">
                                <ESparkline
                                    data={recentSnapshots.slice(0, 10).map(s => s.energy).reverse()}
                                    width={300}
                                    height={80}
                                    color="#22c55e"
                                    showTooltip={true}
